...
def test_solve_return_empty_array():
	""" x^2+1 = 0 """
	assert solve(a=1.0, b=0.0, c=1.0) == []
	

def test_solve_return_two_roots():
	""" x^2-1 = 0 """
	assert solve(a=1.0, b=0.0, c=-1.0) == [1.0, -1.0]
	
	
def test_solve_return_one_root():
	""" x^2+2x+1 = 0 """
	assert solve(a=1.0, b=2.0, c=1.0) == [-1.0]


def test_solve_raise_zero_division_error():
	""" raise ZeroDivisionError """
	with pytest.raises(ZeroDivisionError):
		solve(a=0.0, b=2.0, c=1.0)
		

def test_solve_raise_type_error():
	""" raise TypeError """
	with pytest.raises(TypeError):
		solve(a=1.0, b=2.0, c="1.0")  # type: ignore
	
	
def test_solve_discriminant_was_less_than_the_specified_epsilon():
